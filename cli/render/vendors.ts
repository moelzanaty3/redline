...
// OpenAI Codex reads AGENTS.md — the same file the `agents` vendor writes.
//
// It is a separate vendor id because an operator looking for Codex has to be
// able to find it by name, and "it is covered by the one called agents" is not
// something a menu can say. It is the SAME renderer because inventing a
// Codex-specific path would put the rules somewhere nothing reads them: a file
// only Redline believes in is worse than no file. Selecting both writes
// AGENTS.md once, with identical content either way.
const codex: VendorRenderer = (ctx) => agents(ctx);

export const VENDORS: Record<string, VendorRenderer> = { copilot, agents, codex, claude, cursor };
