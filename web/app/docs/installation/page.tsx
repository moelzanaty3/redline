  ...
  return (
    ...
    >
      ...
      <ul>
        ...
        <li>A git repository whose remote points at GitHub (including GitHub Enterprise Server, if the hostname contains <code>github</code>) or Azure DevOps.</li>
        <li>No install step for the CLI itself — <code>npx redline-cli</code> fetches it on demand. The npm package is <code>redline-cli</code>; the command it installs is <code>redline</code>.</li>
      </ul>
      ...
      <p>
        ...
        pinned to a full commit SHA; <code>scripts/check-pins.mjs</code> keeps
        it honest.
      </p>
      <p>
      ...
      <ul>
        <li>A self-hosted GitHub Enterprise Server on a hostname that doesn&apos;t contain <code>github</code> is not auto-detected.</li>
        <li>There is no <code>redline sync</code> yet — standards distribution to already-onboarded repos beyond the initial pull request arrives in a later phase.</li>
        <li>No offline single-file executable yet, which matters for air-gapped Azure agents without npm registry access.</li>
        <li>The merge gate installs advisory-only on both hosts. Promoting it to blocking is a deliberate second step, after a soak period.</li>
      </ul>
    </DocsPage>
  );
}
